...
describe('enemy AI', () => {
  it('stays idle until the player is inside the aggro range', () => {
    const e = createEnemy('e', 'tycoon', { x: 0, z: 0 });
  ...
  it('chases at its own speed and faces the player', () => {
    ...
    expect(e.pos.x).toBeLessThan(0.5 * ENEMIES.tycoon.speed + 0.5);
    expect(e.facing.x).toBeCloseTo(1);
  });

  it('winds up, then strikes once with the melee damage, then recovers', () => {
    const e = createEnemy('e', 'speaker', { x: 0, z: 0 });
    const c = ctx({ x: 2, z: 0 });
    e.state = 'chase';
    const actions = run(e, c, 0.9);
    expect(actions).toHaveLength(1);
    expect(actions[0]).toMatchObject({ kind: 'melee', damage: ENEMIES.speaker.damage });
    run(e, c, 0.3);
  ...
  it('a stamp golem telegraphs for its whole windup before striking', () => {
    ...
    const early = run(e, c, ENEMIES.speaker.windup - 0.1);
    expect(early).toHaveLength(0);
    expect(e.state).toBe('windup');
  });

  it('a ranged enemy keeps its distance and fires a projectile toward the player', () => {
    const e = createEnemy('e', 'demagogue', { x: 0, z: 0 });
    e.state = 'chase';
    const c = ctx({ x: 2, z: 0 });
    run(e, c, 1);
    expect(e.pos.x).toBeLessThan(0);
    const e2 = createEnemy('e2', 'demagogue', { x: 0, z: 0 });
    e2.state = 'chase';
    const shots = run(e2, ctx({ x: 6.5, z: 0 }), 1.2);
    expect(shots.length).toBeGreaterThanOrEqual(1);
    expect(shots[0]).toMatchObject({ kind: 'projectile', damage: ENEMIES.demagogue.damage });
  ...
  it('damage interrupts light enemies but a windup golem has armor', () => {
    const wisp = createEnemy('w', 'tycoon', { x: 0, z: 0 });
    wisp.state = 'windup';
    wisp.timer = 0.2;
    damageEnemy(wisp, 5, { x: 1, z: 0 }, 2, 0);
    expect(wisp.state).toBe('stunned');
    const golem = createEnemy('g', 'speaker', { x: 0, z: 0 });
    golem.state = 'windup';
    golem.timer = 0.5;
    damageEnemy(golem, 5, { x: 1, z: 0 }, 2, 0);
    expect(golem.state).toBe('windup');
    expect(golem.hp).toBe(ENEMIES.speaker.maxHp - 5);
  });
  ...
  it('dies at zero health, reports the kill once, and respawns full', () => {
    const e = createEnemy('e', 'tycoon', { x: 3, z: 3 });
    ...
    respawnEnemy(e);
    expect(e.hp).toBe(ENEMIES.tycoon.maxHp);
    expect(e.state).toBe('idle');
    expect(e.pos).toEqual({ x: 3, z: 3 });
  });

  it('a blinded enemy stumbles around and its swings are flagged blind', () => {
    const e = createEnemy('e', 'speaker', { x: 0, z: 0 });
    blindEnemy(e, 5);
    const c = ctx({ x: 1.5, z: 0 }, 1);
    const actions = run(e, c, 3);
    expect(actions.length).toBeGreaterThanOrEqual(1);
    for (const a of actions) expect(a.blind).toBe(true);
    expect(e.state).toBe('blinded');
    expect(Math.hypot(e.pos.x, e.pos.z)).toBeGreaterThan(0.5);
    stepEnemy(e, ctx({ x: 1.5, z: 0 }, 5.1), DT);
    expect(e.state).toBe('idle');
  });

  it('a blinded ranged enemy fires in random directions, not at the player', () => {
    const e = createEnemy('e', 'demagogue', { x: 0, z: 0 });
    blindEnemy(e, 99);
    const shots = run(e, ctx({ x: 6, z: 0 }, 1), 12).filter((a) => a.kind === 'projectile');
    expect(shots.length).toBeGreaterThanOrEqual(5);
    const towardPlayer = shots.filter((a) => a.kind === 'projectile' && a.dir.x > 0.95);
    expect(towardPlayer.length).toBeLessThan(shots.length / 2);
    const angles = new Set(
      shots.map((a) => (a.kind === 'projectile' ? Math.round(a.dir.x * 4) : 0)),
    );
    expect(angles.size).toBeGreaterThan(2);
  });

  it('blind behavior is deterministic per enemy id', () => {
    const trace = (id: string) => {
      const e = createEnemy(id, 'tycoon', { x: 0, z: 0 });
      blindEnemy(e, 99);
      run(e, ctx({ x: 9, z: 9 }, 1), 3);
      return e.pos;
    };
    expect(trace('same')).toEqual(trace('same'));
    expect(trace('same')).not.toEqual(trace('different'));
  });

  it('damage does not snap a blinded enemy out of its state', () => {
    const e = createEnemy('e', 'tycoon', { x: 0, z: 0 });
    blindEnemy(e, 99);
    damageEnemy(e, 5, { x: 1, z: 0 }, 2, 0);
    expect(e.state).toBe('blinded');
    expect(e.hp).toBe(ENEMIES.tycoon.maxHp - 5);
  });
  ...
  it('stands down when told to and ignores a fainted player', () => {
    ...
    standDown(e);
    expect(e.state).toBe('returning');
    const f = createEnemy('f', 'tycoon', { x: 0, z: 0 });
