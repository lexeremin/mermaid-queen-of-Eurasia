...
import { clampToRange, pickTeleportDestination } from '@/systems/teleport';
...
describe('pickTeleportDestination', () => {
  it('lands exactly on a free aim point', () => {
    expect(pickTeleportDestination(from, { x: -8, z: 3 }, 14, world, anywhere)).toEqual({
      x: -8,
      z: 3,
    });
  });
  ...
  it('jumps over a wall to the free ground behind it', () => {
    const dest = pickTeleportDestination(from, { x: 10, z: 0 }, 14, world, anywhere)!;
    expect(dest).toEqual({ x: 10, z: 0 });
  });

  it('falls back to the nearest free spot toward Rosa when the aim is inside something', () => {
    const dest = pickTeleportDestination(from, { x: 6, z: 0 }, 14, world, anywhere)!;
  ...
  it('never lands beyond its range', () => {
    const dest = pickTeleportDestination(from, { x: -40, z: 0 }, 14, world, anywhere)!;
  ...
  it('refuses another region, stopping at the last place she could walk to', () => {
    ...
    const dest = pickTeleportDestination(from, { x: 12, z: 0 }, 14, world, sameSide)!;
    expect(dest.x).toBeLessThan(5);
  });

  it('gives up when there is nowhere to go', () => {
    expect(pickTeleportDestination(from, { x: 0.3, z: 0 }, 14, world, anywhere)).toBeNull();
    expect(pickTeleportDestination(from, { x: 12, z: 0 }, 14, world, () => false)).toBeNull();
  });
  ...
  it('stays inside the map bounds', () => {
    ...
    const dest = pickTeleportDestination(from, { x: 12, z: 0 }, 14, tight, anywhere)!;
    expect(dest.x).toBeLessThan(5);
  });
});
