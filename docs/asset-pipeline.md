...
- Consequence: faces are flat colors. Chunky faceted geometry + palette + fog gives the retro look. Pixel-art surface detail is **not** possible with single-texel UVs, so large ground surfaces use small **tiling detail textures** instead. Current tiles: `cobble_tile.png` (wet cobblestone, one tile per 2 m) and `grass_tile.png` (one tile per 3 m), both 32×32 in palette colors, applied with `RepeatWrapping` and nearest filtering (`TiledGround` in `MapScene.tsx`). Add more tiles the same way if a surface needs them (a dither post effect is still an option for the polish phase).
