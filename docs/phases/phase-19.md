...
- **Loading screens:** the loading screen is only shown while the page loads (a browser reload). Moving between areas (metro, stairs, Recall) is instant (no fade, no overlay, just a burst of bubbles at both ends); nothing else blocks the game.
- **Performance:**
