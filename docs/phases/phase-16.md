...
- **Blink (superseded by 16c below: now on Shift, replaces Dash):** a new ability like the Wizard's Teleport in Diablo 3: instantly moves Rosa toward the mouse cursor (or the way she moves or faces on touch), up to 14 m, over walls and props, 20 mana, 6 s cooldown, a moment of invulnerability, bubbles at both ends. It only lands where she could walk to (`NavGrid.sameRegion`), so it cannot skip the shut boss gate or enter the underground from the surface; if the aim is blocked it lands on the nearest free spot toward her.

## Follow-up (16c): Blink replaces Dash, Recall, HUD icons
- **Blink replaces Dash** in the skill bar (Shift, "BLINK" on touch): 15 mana, 4 s cooldown, 14 m, over walls, region-safe, 0.35 s of invulnerability. The dash (movement override, effects, icon, tests) is gone. The boss balance test now dodges with Blink: a level-5 bot still wins with roughly 80 of 140 health left.
- **Recall** (`T`, small round icon in the corner dock, not on the skill bar): a 3 s channel that returns Rosa to the game start on Red Square (from anywhere, including the underground, with the usual fade). Standing still is required: moving, click-walking, attacking, using another skill, taking damage, fainting, using a place (metro, stairs) or pressing `T` again cancels it. A ring fills around the icon and a column of light rises around Rosa. Leaving the underground this way refills it on the next descent like any exit.
- **Icons instead of text buttons:** Quests, Bag and Menu are square icon buttons (scroll, satchel, pause badge) with the key hint (J, I, Esc) on desktop.
- **HUD layout:** the follower's Talk button, the Recall icon and the potions now live in one bottom-corner dock that stacks them (a column on the left above the stick on touch, a row on landscape phones), so they cannot overlap. Narrow phones get a smaller stick and a scaled button cluster; landscape phones get a shorter boss bar and a lower prompt. A scripted audit (five viewports from 1280x800 to 320x568 and a landscape phone, with a follower, toasts, the boss bar, a prompt and the zone banner) finds no overlaps; the one exception is the 3 s zone banner sharing space with a toast on a 320 px wide screen.

## Follow-up (16d): vintage HUD, welcome screen, controls popup
- **Layout:** on desktop the corner dock (Recall, potions, and the follower's Talk button) moved to the bottom-left; on touch, Recall floats above SPELL near the right edge with a clear gap (potions and Talk stay on the left above the stick).
- **Prince Sasha's avatar** replaces the letter on his Talk button (and in his dialogue box): `public/assets/ui/avatar_prince.png`, rendered from his own model (ledger row added, `src/data/avatars.ts`).
- **One vintage style, kept minimal:** dark leather panels with a gold rim and a thin inner line, a serif face (system Georgia/Palatino, no downloads), squarer corners, shaded red, blue and gold bars, small gold ornament rules under titles. It is applied by mapping the old colours onto shared tokens (`--ui-*` in `hud.css`) plus one theme block at the end, so the HUD, the bag, quests, dialogue, prompts, toasts, banners, the boss bar and the menus all match.
- **Welcome screen** (`Welcome.tsx`) on a brand new game (no save) and after NEW GAME: the mermaid, the title, a short story, BEGIN and CONTROLS. The world waits behind it. (Dev only: `?welcome=0` skips it for scripted runs.)
