...
Events sent today: `session_started {device}`, `npc_method_used {npc, method}`, `npc_mesmerized {npc}`, `npc_joined {npc}`, `enemy_defeated {kind}`, `player_downed`, `level_up {level}`, `quest_accepted {id}`, `quest_completed {id}`, `shrine_gift`.
...
- **Cloud save:** after sign-in, pull once; apply only if `saved_at` is newer than the local save (ties keep local). Push when progress (form, NPC state, level, XP, bag, equipment, quests, garden) changed: at most every 30 s, and immediately when an NPC joins or the tab hides. Position alone never triggers a push.
