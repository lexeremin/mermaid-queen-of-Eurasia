...
**Phase 27 complete (real-device QA preparation; the on-device pass is the owner's, `docs/qa-iphone.md`).** Next: **Phase 28 — Deploy** (`docs/phases/phase-28.md` — write it first, it does not exist yet).
...
3. Read `docs/phases/phase-28.md` for the specific task. If the doc for the current phase does not exist yet, write it first (goal, scope, steps, DoD) and add it to the commit.
...
| Phase 25 — Polish | ✅ Complete | Master volume and ambience sliders, twelve new synthesized sounds wired into UI, level-up, quests, loot, hits, blink, swimming and bosses, generative ambience per scene, drizzle and mist, floating damage numbers, screen shake, "Welcome back" title screen | 3cc2816 |
| Phase 26 — Skills and sixty levels | ✅ Complete | Abilities unlock one by one (Trident 1, Aura 3, Blink 6, Surge 10), level cap 60, passives every tenth level (sea wave 20, Aura light beams 30, Surge sharks 40, Blink arcane blast 50), holy wings and halo at 60, a much steeper dungeon, richer Surge visuals, softer noise | d5d8031 |
| Phase 27 — Real-device QA | ✅ Complete | Phone-emulation audit of every screen (menus fit sideways and on the SE, dialogue choices, recall button size), Safari 15 build target, geometry leak fixed, offline and soak checks, `?perf=1` on-device report, `docs/qa-iphone.md` checklist. The real iPhone pass is left to the owner | 94124ee |
| Phase 28 — Deploy | ⬜ Next | Hosting (Cloudflare Pages or Vercel), env config, release | |
