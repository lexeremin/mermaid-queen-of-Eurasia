...
| Phase 27 — Real-device QA | ✅ Complete | Phone-emulation audit of every screen (menus fit sideways and on the SE, dialogue choices, recall button size), Safari 15 build target, geometry leak fixed, offline and soak checks, `?perf=1` on-device report, `docs/qa-iphone.md` checklist. The real iPhone pass is left to the owner | |
