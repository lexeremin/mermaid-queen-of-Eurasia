...
**Phase 5 complete (hero redesign + direction change).** Next: **Phase 6 — Red Square map** (`docs/phases/phase-6.md` — write it first, it does not exist yet).
...
3. Read `docs/phases/phase-6.md` for the specific task. If the doc for the current phase does not exist yet, write it first (goal, scope, steps, DoD) and add it to the commit.
...
| Phase 6 — Red Square map | ⬜ Next | Replace the village: cathedral, Kremlin-style wall, GUM-style arcade, museum, cobblestone plaza, river + bridge, Alexander Garden entrance | Cobble tile texture; retire village-only assets |
